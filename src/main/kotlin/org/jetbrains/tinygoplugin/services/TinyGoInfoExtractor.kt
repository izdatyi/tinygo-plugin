...
suspend fun TinyGoConfiguration.extractTinyGoInfo(msg: String) {
    ...
    try {
        ...
        TinyGoInfoExtractor.logger.error(
            "error while extracting parameters from tinygo command output",
            e,
            "process output: \"$msg\"\n",
        )
    }
...
class TinyGoExecutable(
    ...
    suspend fun execute(
        ...
        val executor =
            createExecutor(sdkRoot, arguments, showErrors)
                ?.withProcessListener(processHistory)
                ?: return null
        return suspendCancellableCoroutine { continuation ->
            val task =
                executor.createTask(true, showErrors, null, null, null) { execution ->
...
class TinyGoInfoExtractor(
    ...
    private fun tinyGoExtractionArguments(settings: TinyGoConfiguration): List<String> = listOf("info") + tinyGoArguments(settings)
    ...
    private suspend fun reportFailure(
        ...
        withContext(Dispatchers.EDT) {
            ...
            val errorMessage =
                incompatibleVersionErrorMessage ?: TinyGoBundle.message(DETECTION_ERROR_MESSAGE).also {
